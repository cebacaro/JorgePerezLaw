...
const Page = () => {
  ...
  return (
    <div className="relative w-full min-h-screen overflow-hidden flex justify-center">
      ...
      />
      <section className="absolute inset-0 flex flex-col items-center justify-center backdrop-filter backdrop-blur-md bg-opacity-20 bg-black z-10 min-h-screen w-screen">
        <div className="grid grid-cols-1 h-[80vh] px-4 font-extralight md:grid-cols-2 gap-2 tx-sm md:text-sm lg:mx-20 overflow-auto  ">
          {reviews.map((review, index) => (
            <div
              key={index}
              className="p-4   border shadow-md shadow-[#ceb475]/50 rounded h-auto"
            >
